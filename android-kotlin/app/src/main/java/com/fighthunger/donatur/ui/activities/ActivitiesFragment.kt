...
class ActivitiesFragment : Fragment() {
    private var _b: FragmentActivitiesBinding? = null
    private val b get() = _b!!

    override fun onCreateView(i: LayoutInflater, c: ViewGroup?, s: Bundle?): View {
        _b = FragmentActivitiesBinding.inflate(i, c, false)
        return b.root
    }

    override fun onViewCreated(v: View, s: Bundle?) {
        b.activityList.layoutManager = LinearLayoutManager(requireContext())
        lifecycleScope.launch {
            val uid = FirebaseAuth.getInstance().currentUser?.uid ?: return@launch
            val items = DonationRepository().history(uid)
            b.activityList.adapter = ActivityAdapter(items)
        }
    }
